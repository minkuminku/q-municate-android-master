package com.quickblox.q_municate.ui.nearbyfriends;
...
public class NewFriendDetailsActivity extends BaseLogeableActivity {
    ...
    public static NewFriendsListFragment newInstance() {
        return new NewFriendsListFragment();
    }

    public static void start(Context context, int friendId) {
        Intent intent = new Intent(context, NewFriendDetailsActivity.class);
    ...
    private void registerStatusChangingObserver() {
        statusContentObserver = new ContentObserver(new Handler()) {
            ...
            public void onChange(boolean selfChange) {
                if (NewFriendDetailsActivity.this.user != null) {
                    user = UsersDatabaseManager.getUserById(NewFriendDetailsActivity.this,
                            NewFriendDetailsActivity.this.user.getUserId());
    ...
    private void showRemoveUserDialog() {
        ...
        alertDialog.setPositiveButton(new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                showProgress();
                QBRemoveFriendCommand.start(NewFriendDetailsActivity.this, user.getUserId());
            }
        });
    ...
    private void callToUser(User friend, com.quickblox.videochat.webrtc.Consts.MEDIA_STREAM callType) {
        if (friend.getUserId() != AppSession.getSession().getUser().getId()) {
            if (checkFriendStatus(friend.getUserId())) {
                CallActivity.start(NewFriendDetailsActivity.this, friend, callType);
            }
        }
    }
        ...
        if (checkFriendStatus(user.getUserId())) {
            try {
                QBDialog existingPrivateDialog = privateChatHelper.createPrivateDialogIfNotExist(
                        user.getUserId());
                PrivateDialogActivity.start(NewFriendDetailsActivity.this, user, existingPrivateDialog);
    ...
    private class RemoveFriendSuccessAction implements Command {
        ...
        public void execute(Bundle bundle) {
            deleteDialog();
            DialogUtils.showLong(NewFriendDetailsActivity.this, getString(R.string.dlg_friend_removed,
                    user.getFullName()));
            finish();
        }
    }
